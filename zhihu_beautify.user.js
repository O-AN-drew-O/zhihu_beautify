...
// @version      2.1.2.1
// @description  1.【重要更新】增加夜间模式按钮     2.知乎题目栏增加举报、匿名、问题日志、快捷键四个按钮     3.知乎按钮图标在鼠标悬停时变色(题目按钮、回答下方按钮、评论按钮等)     4.回答的发布时间移至顶部     5.图片原图显示     6.文字和卡片链接从知乎跳转链接改为直链     7.集成其他脚本的知乎视频下载功能     8.隐藏侧边栏     9.GIF图自动播放【默认不开启】     10.问题增加创建时间和最后编辑时间     11.鼠标悬停在回答时显示浅蓝色聚焦框    12.角标注释高亮    13.首页信息流增加不感兴趣按钮  14.【重要更新】增加设置界面
// @author       AN drew
// @match        *://*.zhihu.com/*
...
function iconColor() {
    ...
    $(".Zi--Star").parent().parent().hover(function() {
        if (!$(this).hasClass("ExploreHomePage-ContentSection") && !$(this).hasClass("css-18biwo")) {
...
*/
    ...
    $('.ztext sup[data-draft-type=reference]').click(function() {
        ...
        let ref_id = $(this).find('a').attr('id').substring(0, $(this).find('a').attr('id').lastIndexOf('_'));
...
function addCSS() {
    var css = 'html[data-theme=dark] .css-1qefhqu{background-color:#1A1A1A}' +
        ...
        'html[data-theme=dark] .VideoGallery-root-7Z1Ci{background:rgb(18,18,18)}' +
...
function settings() {
    $('body').append('<div id="settingLayerMask" style="display: flex;">' +
        ...
        '				<select name="hideRecommendedReading" id="hideRecommendedReading">' +
        '							<option value="0">不隐藏</option>' +
        ...
        '			</section>' +
        '			<section class="switch" style="visibility:hidden"><span>none2</span>' +
        '				<div class="checkbox on"><input type="checkbox" name="none2" id="none2" value="1"><label class="switchLabel"></label></div>' +
        '			</section>' +
        '		</div>' +
        '		<div id="btnEle">' +
